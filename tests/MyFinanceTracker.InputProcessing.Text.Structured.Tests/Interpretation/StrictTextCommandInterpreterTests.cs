...
namespace MyFinanceTracker.InputProcessing.Text.Structured.Tests.Interpretation;
...
public class StrictTextCommandInterpreterTests
{
    ...
    async Task Interpret_AddTransactionCommand_ReturnsIdentifiedWithCorrectPayload(string input, string expectedPayload)
    {
        // act
        var result = await interpreter.Interpret(InterpretationInput.Create(input));
    ...
    [Fact]
    async Task Interpret_DeleteTransactionCommand_ReturnsIdentifiedWithCorrectPayload()
    {
        // arrange
    ...
    [Fact]
    async Task Interpret_ListCategoriesCommand_ReturnsIdentifiedWithoutPayload()
    {
        // arrange
        var input = "c all";
